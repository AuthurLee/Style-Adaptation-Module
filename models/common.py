...
class Transfer_BottleNeck(nn.Module):
    # Bottleneck layers for domain adaptation
    def __init__(self, c1, c2):  # ch_in, ch_out
        super(Transfer_BottleNeck, self).__init__()
        self.model = nn.Sequential(
            nn.AdaptiveAvgPool2d((10, 10)),
            nn.Conv2d(c1, c2, 1),
            nn.Flatten(),
            nn.BatchNorm1d(c2 * 10 * 10),
            nn.ReLU(),
            nn.Dropout(0.5)
        )
...
class Style(nn.Module):
    # Take the channel-wise mean and variance as the style of the domain 
    def __init__(self, c1, c2):  # c1 is ch_in, c2 is ch_out
    ...
    def forward(self, x):
        out = self.conv(x)
        # Calculate the channel-wise mean and variance
        mean = torch.mean(out, dim=(2, 3))
        var = torch.var(out, dim=(2, 3), unbiased=False)
        out = torch.concat([mean.unsqueeze(1), var.unsqueeze(1)], dim=1)
        return out
    
class Style_v2(nn.Module):
    # Take the channel-wise mean and variance as the style of the domain 
    def __init__(self, c1, c2):  # c1 is ch_in, c2 is ch_out
        super(Style_v2, self).__init__()
        # self.conv = nn.Conv2d(c1, c2, kernel_size=1)
        self.conv = nn.Sequential(
            nn.Conv2d(c1, c2, kernel_size=1),
            nn.BatchNorm2d(c2)
...
class DomainRep(nn.Module):
    # domain representation
    # Linear combination of previous bottleneck layers for domain adaptation
    def __init__(self):
        super(DomainRep, self).__init__()
        self.linear_comb = nn.Conv1d(3, 1, kernel_size=1)
    def forward(self, x1, x2, x3):
        x = torch.concat([x1.unsqueeze(1), x2.unsqueeze(1), x3.unsqueeze(1)], dim=1)
        out = self.linear_comb(x).squeeze()
        return out

class StyleRep_single(nn.Module):
    # style representation for only one source (certain depth of features in backbone)
    def __init__(self):
        super(StyleRep_single, self).__init__()
        self.linear_comb = nn.Conv1d(2, 1, kernel_size=1)
    def forward(self, x):
        out = self.linear_comb(x).squeeze()
        return out
...
class StyleRep(nn.Module):
    ...
    # Linear combination of Style module
    def __init__(self):
        super(StyleRep, self).__init__()
        self.linear_comb = nn.Conv1d(6, 1, kernel_size=1)
        self.bn = nn.BatchNorm1d(num_features=1)
    def forward(self, x1, x2, x3):
        x = torch.concat([x1, x2, x3], dim=1)
        # out = self.linear_comb(x).squeeze()
        out = self.linear_comb(x)
        out = self.bn(out).squeeze()
        return out


class StyleRep_sum(nn.Module):
    # Style representation
    # Linear combination of Style module
    def __init__(self, n_channel):
        super(StyleRep_sum, self).__init__()
        # self.linear_comb = nn.Conv1d(6, 1, kernel_size=1)
        self.bn = nn.BatchNorm1d(num_features=n_channel)
    def forward(self, x1, x2, x3):
        # x = torch.concat([x1, x2, x3], dim=1)
        # print(x1.size())
        x = x1 + x2 + x3
...
class StyleRep_mlp(nn.Module):
    # Style representation
    # Linear combination of Style module
    def __init__(self, d_model):
        super(StyleRep_mlp, self).__init__()
        # self.linear_comb = nn.Conv1d(6, 1, kernel_size=1)
        self.mlp = nn.Sequential(
            nn.Linear(d_model * 3 * 2, d_model * 3 * 4),
            nn.ReLU(),
    ...
    def forward(self, x1, x2, x3):
        ...
        out = torch.concat([x[:, 0, :], x[:, 1, :]], dim = 1)
        out = self.mlp(out)
        # print(out.size())
        out = self.bn(out.unsqueeze(1)).squeeze()
        return out
