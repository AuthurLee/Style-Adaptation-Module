...
def representation_save(x, module_type, stage, save_dir=Path('runs/detect/exp')):
    """
    x:              Feature maps to be saved
    ...
    file_name = f"{stage}_0"
    path = os.path.join(save_dir, f"{file_name}.pt")
    while os.path.exists(path):
        file_name = f"{stage}_{str(int(file_name.split('_')[1]) + 1)}"
        path = os.path.join(save_dir, f"{file_name}.pt")
    torch.save(x, path)
