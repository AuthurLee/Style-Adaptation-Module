...
class MMD_loss(nn.Module):
    def __init__(self, kernel_type='gaussian', kernel_param=1.0):
        super(MMD_loss, self).__init__()
        self.type = kernel_type
        self.param = kernel_param
    
    def forward(self, x, y):
        loss = self.compute_mmd(x, y, self.type, self.param)
        return loss

    def compute_mmd(self, x, y, kernel_type='gaussian', kernel_param=1.0):
        xx = self.pairwise_kernel(x, x, kernel_type, kernel_param)
        yy = self.pairwise_kernel(y, y, kernel_type, kernel_param)
        xy = self.pairwise_kernel(x, y, kernel_type, kernel_param)

        mmd = torch.mean(xx) - 2 * torch.mean(xy) + torch.mean(yy)
        return mmd
        
    def pairwise_kernel(self, x, y, kernel_type='gaussian', kernel_param=1.0):
        if kernel_type == 'gaussian':
            dist = torch.sum((x.unsqueeze(1) - y.unsqueeze(0)) ** 2, dim=-1)
            kernel_matrix = torch.exp(-0.5 * dist / kernel_param**2)
        elif kernel_type == 'linear':
            kernel_matrix = torch.matmul(x, y.t())
        elif kernel_type == 'polynomial':
            kernel_matrix = (torch.matmul(x, y.t()) + 1) ** kernel_param
        else:
            raise ValueError("Unsupported kernel type: {}".format(kernel_type))
        return kernel_matrix

class Norm2Diff(nn.Module):
    def __init__(self):
        super(Norm2Diff, self).__init__()
    def forward(self, source, target):
        return torch.norm(source - target)

class Norm1Diff(nn.Module):
    def __init__(self):
        super(Norm1Diff, self).__init__()
    def forward(self, source, target):
        loss_fn = nn.L1Loss()
...
class KLDiv(nn.Module):
    ...
    def forward(self, source, target):
        ...
        loss = (loss_fn(torch.log(source), target) + loss_fn(torch.log(target), source)) / 2
        return loss
...
class CORAL_loss(nn.Module):
    ...
    def forward(self, source, target, **kwargs):
        source_covariance = torch.matmul(source.T, source) / source.size(0)
        target_covariance = torch.matmul(target.T, target) / target.size(0)

        source_mean = torch.mean(source, dim=0, keepdim=True)
        target_mean = torch.mean(target, dim=0, keepdim=True)

        loss = torch.mean((source_covariance - target_covariance) ** 2)
        loss += torch.mean((source_mean - target_mean) ** 2)

        return loss
    
    
        # # source: (num_samples, d) 
        # # the shape of the correlation matrix is (d, d)
        # d = source.data.shape[1]
        # ns, nt = source.data.shape[0], target.data.shape[0]

        # # source covariance
        # # here, in torch.mean(), if keepdim=True, the size of the output is the same as the input
        # # xm.t() is the transpose of xm, @ is the operator for matrix multiplication
        ...
        # # '/(d*d)' is to reduce the effect of matrix shape size on the final results of loss 
        # # Another codes for frobenius norm
